...
;
(defun not-colors (n c1 c2 k)
    (if (> c2 k)
        nil
        (cons (list (- (node2var n c1 k)) (- (node2var n c2 k))) 
              (not-colors n c1 (+ c2 1) k))
    )
)
...
(defun at-most-one-color (n c k)
    (if (> c k)
        nil
        (append (not-colors n c (+ c 1) k) (at-most-one-color n (+ c 1) k))
    )
)
...
(defun edge-helper (e c k)
    (if (> c k)
        nil
        (cons (list (- (node2var (car e) c k)) (- (node2var (cadr e) c k)))
              (edge-helper e (+ c 1) k))
    )
)
